# AIML
Rev. 85 | Created: 2026-08-07 | Updated: 2026-09-21 10:07 CDT
...
| [Agile](Agile/) | It covers the way the work around the model is run rather than the model itself — the three layers of agile practice, the sprint cycle and the vocabulary a team says its status in under `Agile-Software-Development/`, what changes when the deliverable is a model rather than a feature under `Agile-AIML-Development/`, and the meeting that judges a modeling result under `AIML-Code-Review-Meeting/`. |
...
| [Agile/AIML-Code-Review-Meeting/aiml-code-review-meeting.md](Agile/AIML-Code-Review-Meeting/aiml-code-review-meeting.md) ([Korean](Agile/AIML-Code-Review-Meeting/aiml-code-review-meeting-ko.md)) | It places a review finding at one of three levels by how many people it needs, leaving the meeting only the judgement a pull request cannot make, fixes the packet an author circulates a day ahead, walks a timeboxed agenda from framing through data, experiment, code and serving to a decision, lists for each stage the check the room runs and the failure that looks like success, and closes on the four outcomes a meeting may issue and what each one records. |

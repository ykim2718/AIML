# AIML
Rev. 87 | Created: 2026-08-07 | Updated: 2026-09-21 10:29 CDT
...
| [Agile/AIML-Modeling-Meeting/aiml-modeling-meeting.md](Agile/AIML-Modeling-Meeting/aiml-modeling-meeting.md) ([Korean](Agile/AIML-Modeling-Meeting/aiml-modeling-meeting-ko.md)) | It names the ten things a modeling meeting exchanges — target, provenance, baseline, hypothesis, run, evidence, insight, readiness, verdict and handoff — groups them into the four classes that are also the agenda, and grades every number on a five-step ladder from an assertion nobody can check to a result compared against the model already serving. It gives the required grade for each term, the sentence that closes each stage, the missing term behind each of the three habits that end a meeting without a verdict, the class each role supplies, and a three-line record filled from one cycle. |

# AIML
Rev. 86 | Created: 2026-08-07 | Updated: 2026-09-21 10:20 CDT
...
| [Agile/AIML-Modeling-Meeting/aiml-modeling-meeting.md](Agile/AIML-Modeling-Meeting/aiml-modeling-meeting.md) ([Korean](Agile/AIML-Modeling-Meeting/aiml-modeling-meeting-ko.md)) | It stacks what a modeling meeting decides into five layers, from the definition of the target up to the next action, so that a meeting opened above an unsettled layer is recognised before it produces an action nobody can evaluate; it then runs the meeting in four stages — data and baseline, hypothesis, experiment review, next actions — names the three habits that end a meeting without a decision, divides the room between the domain expert, the data scientist and the MLOps engineer, and closes on a three-line minutes template filled from one cycle. |

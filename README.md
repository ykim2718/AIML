# AIML
Rev. 66 | Created: 2026-08-07 | Updated: 2026-09-10 23:55 UTC
...
| [Models/Uncertainty/predictive-uncertainty.md](Models/Uncertainty/predictive-uncertainty.md) ([Korean](Models/Uncertainty/predictive-uncertainty-ko.md)) | It sets out the ways of attaching a probability to a predicted value as a taxonomy of three axes — what the model emits per row, which term of the predictive variance that emission is made of, and how it is obtained — and draws the first axis as a hierarchy of four levels: a point, an interval at one stated level, a quantile function whose width moves with the input, and a distribution that answers what the probability is of passing any stated limit. It records for eight linear regressors and five tree models what each library emits and which parameter changes it, separates the level an answer reaches from whether its coverage holds, and measures eleven methods against one another on one held-out split, where a constant-width interval covers 0.908 of all rows while covering 1.000 of the quietest third and 0.785 of the noisiest. |

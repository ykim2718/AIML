# AIML
Rev. 64 | Created: 2026-08-07 | Updated: 2026-09-10 21:29 UTC
...
| [Models/Ensemble/Ensemble-Learning/ensemble-learning.md](Models/Ensemble/Ensemble-Learning/ensemble-learning.md) ([Korean](Models/Ensemble/Ensemble-Learning/ensemble-learning-ko.md)) | It separates the rule that combines fitted models from the framework that produces them, derives the ambiguity decomposition and the correlation floor that bound what any combination can buy, and works through voting, averaging, weighted averaging, stacking and blending against each other on one held-out split. Its subject is what a member emits beyond its label — the probability and the confidence — with the calibration that has to come first, the four places that number enters a combination, and the measured finding that a meta-learner fed probabilities beats the same stack fed labels, while averaging those same probabilities does not. |

# AIML
Rev. 88 | Created: 2026-08-07 | Updated: 2026-09-21 10:38 CDT
...
| [Agile](Agile/) | It covers the way the work around the model is run rather than the model itself — the three layers of agile practice, the sprint cycle and the vocabulary a team says its status in under `Agile-Software-Development/`, what changes when the deliverable is a model rather than a feature under `Agile-AIML-Development/`, and the weekly meeting that closes one experiment and opens the next under `Agile-AIML-Modeling-Meeting/`. |
...
| [Feature-Engineering/Feature-Selection/README.md](Feature-Engineering/Feature-Selection/README.md) | It indexes the Feature-Selection folder, which divides the methods by what the selection looks at — one feature at a time, several features together, or the trace columns of semiconductor equipment. |
| [Feature-Engineering/Feature-Selection/trace-feature-selection/trace-feature-selection.md](Feature-Engineering/Feature-Selection/trace-feature-selection/trace-feature-selection.md) | It places the methods that choose which of a semiconductor equipment trace's features move the target on three axes — when the model is consulted, which sorts the filter, wrapper, embedded, post-hoc and error-controlled branches and fixes where attribution and attention sit against a training penalty; the unit the selection is made at, which decides whether a selected set names a sensor or a step a line can act on; and whether the selection survives a change of wafers — and closes on a question-to-branch guide and the failure modes the branches share. |
| [Feature-Engineering/Feature-Selection/trace-feature-selection/trace-feature-selection-ko.md](Feature-Engineering/Feature-Selection/trace-feature-selection/trace-feature-selection-ko.md) | It is the Korean edition of the trace feature selection document, carrying the same hierarchy, axes, tables and references, with the headings and captions left in English. |
| [Feature-Engineering/Feature-Selection/univariate-feature-selection/univariate-feature-selection-ko.md](Feature-Engineering/Feature-Selection/univariate-feature-selection/univariate-feature-selection-ko.md) | It scores each feature alone against the target, giving the statistical test for each combination of continuous and categorical data, the cut-off rules, the two blind spots of scoring one feature at a time, and an appendix that links the `SelectKBest` selector class kept in `src/`, run over the iris dataset under each of the four metrics. |
| [Feature-Engineering/Feature-Selection/multivariate-feature-selection/multivariate-feature-selection.md](Feature-Engineering/Feature-Selection/multivariate-feature-selection/multivariate-feature-selection.md) | It scores features as a subset, splits the methods into multivariate filter, wrapper and embedded, sets that split against the interaction hierarchy, the kind of target and the instability of a single run, and closes on a four-step workflow that puts the expensive method last; an appendix links that workflow as one selector class kept in `src/`, run over the breast cancer dataset, where every step names its method with a Literal declared on the class — four filters, five embedded models, four wrapper searches, all implemented and all reachable from run — and charts which features each of the thirteen keeps, cutting 31 features to 5. |
| [Feature-Engineering/Feature-Selection/multivariate-feature-selection/multivariate-feature-selection-ko.md](Feature-Engineering/Feature-Selection/multivariate-feature-selection/multivariate-feature-selection-ko.md) | It is the Korean edition of the multivariate feature selection document, carrying the same hierarchies, tables, workflow and selector class. |
| [Feature-Engineering/Feature-Selection/BIC/bic.md](Feature-Engineering/Feature-Selection/BIC/bic.md) | It defines the Bayesian information criterion as the deviance of a fit plus a penalty of the log of the sample size per parameter, derives that penalty from the marginal likelihood by the Laplace approximation, gives the linear regression form, the conditions that break it and what a difference between two scores is worth, and closes on four appendices — one carrying that derivation down to the bounded terms it drops, one setting BIC against AIC as an identification criterion against a prediction criterion, one deriving the deviance of a Gaussian linear model from its likelihood, one linking a forward selection run kept in `src/` where BIC stops on the three generating features and AIC on five. |
| [Feature-Engineering/Feature-Selection/BIC/bic-ko.md](Feature-Engineering/Feature-Selection/BIC/bic-ko.md) | It is the Korean edition of the BIC document, carrying the same derivation, tables, worked example and references. |
...
| [Applied-Statistics/Regression/OLS/ordinary-least-squares-ko.md](Applied-Statistics/Regression/OLS/ordinary-least-squares-ko.md) | It is the Korean note on ordinary least squares, building the design matrix from the observations and the model terms, expanding the residual sum of squares and differentiating it to the normal equation, reading that equation geometrically as the residual standing perpendicular to the column space, and giving the closed form the coefficient vector takes when the cross-product matrix is invertible. Two appendices carry the rest: the NumPy computation checked against scikit-learn with the output it prints, and the identity that makes the self inner product of a vector the sum of its squared elements. |
| [Applied-Statistics/Regression/Regularization/regularization-ko.md](Applied-Statistics/Regression/Regularization/regularization-ko.md) | It is the Korean note on regularization, separating the case in which the cross-product matrix has no inverse — more features than observations, or perfect collinearity — from the case in which it has one that is unstable, then setting the ridge, lasso and elastic net penalties against each other by whether they drive a coefficient to zero and by the data each suits, and closing with the two alternatives that remove variables instead of penalising them. An appendix runs the three penalties and ordinary least squares on the same nearly collinear data and shows what each one leaves in the coefficient vector. |
| [Applied-Statistics/Bayesian/Marginal-Likelihood/marginal-likelihood-ko.md](Applied-Statistics/Bayesian/Marginal-Likelihood/marginal-likelihood-ko.md) | It defines the marginal likelihood as the denominator of Bayes theorem and derives it two ways, separates the cases where the integral has a closed form from those where it does not, places the methods that reach it on one hierarchy from conjugate pairs through Laplace and BIC to sampling, and gives the Laplace result with the conditions that break it and its derivation in an appendix. |
| [Applied-Statistics/Bayesian/Bayesian-Statistics/bayesian-statistics-ko.md](Applied-Statistics/Bayesian/Bayesian-Statistics/bayesian-statistics-ko.md) | It splits the classical and the Bayesian school by which side of the conditional probability each computes, works the pool table betting problem through both to the 7:1 and the 10:1 they disagree on, and records the three things — the integrals, the prior, and treating a model as a random variable — that keep the Bayesian answer from being the automatic choice. |
...
| [Agile/Agile-AIML-Modeling-Meeting/agile-aiml-modeling-meeting.md](Agile/Agile-AIML-Modeling-Meeting/agile-aiml-modeling-meeting.md) ([Korean](Agile/Agile-AIML-Modeling-Meeting/agile-aiml-modeling-meeting-ko.md)) | It names the ten things a modeling meeting exchanges — target, provenance, baseline, hypothesis, run, evidence, insight, readiness, verdict and handoff — groups them into the four classes that are also the agenda, and grades every number on a five-step ladder from an assertion nobody can check to a result compared against the model already serving. It places the ten on the agile practices the cycle already runs, where the hypothesis is the sprint backlog item under a work-in-progress limit of one and the required grade is the definition of done, and gives the sentence that closes each stage, the missing term behind each habit that ends a meeting without a verdict, the class each role supplies, and a three-line record filled from one cycle. |

...
Rev. 7 | Created: 2026-08-20 | Updated: 2026-08-22 20:40 UTC
...
## Appendix D. Application to Semiconductor Equipment Signals
...
|-------|---------------|-------------------------|
| White | 열 잡음과 계측 사슬의 전자 잡음 | $M$ 점을 평균하면 $\sqrt{M}$ 만큼 줄어든다 |
| Pink | Sensor 와 증폭기가 가진 1/f 잡음 | 평균해도 더디게 줄어 이득이 곧 한계에 닿는다 |
| Red | 온도와 기준 전압의 이동, sensor 노화 같은 drift | 평균해도 줄지 않고 구간을 늘리면 오히려 커진다 |
| Blue | 장비 신호 자체로는 드물다 | 저주파가 깎인 것이므로 앞단의 처리를 본다 |

이 표의 쓸모는 마지막 열에 있다. White 는 평균이나 filter 로 줄일 수 있으므로 더 오래 재면 해결된다. Red 는 아무리 오래 재도 줄지 않으므로 더 오래 재는 것이 답이 될 수 없고, 원인을 없애거나 주기적으로 다시 맞추어야 한다. 곧 기울기를 모르면 같은 흔들림에 엉뚱한 조치를 하게 된다.

Red 의 물리적 의미는 random walk 이다. 값이 이전 값에서 무작위로 한 걸음씩 옮겨 가므로 시간이 갈수록 처음 값에서 멀어지고, 멀어진 거리의 분산이 지난 시간에 비례하므로 거리 자체는 시간의 제곱근으로 자란다. 그러므로 Red 가 보이는 신호에서는 마지막으로 맞춘 뒤 흐른 시간이 오차의 크기를 정하며, 이것이 calibration 을 다시 하는 주기를 정하는 근거가 된다.

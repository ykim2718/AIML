...
Fig 2 에서 오른쪽 항이 관측 곡선에 겹치기 시작하는 곳을 w2w detection point 라 부르며, 오른쪽 항이 관측값의 98% 를 넘는 첫 $`n`$ 으로 잡으면 이 자료에서는 $`n = 9`$ 이다 ($`n = 8`$ 에서 96%, $`n = 9`$ 에서 98%). w2w detection point 이후 관측 곡선은 사실상 wafer effect 의 산포 그 자체이다.

...
Rev. 79 | Created: 2026-09-01 | Updated: 2026-09-21 20:42 CDT
...
Wafer 를 장당 여러 site 에서 재어 얻은 표를 아래 기호로 적는다. 항목마다 그 값이 어디서 오는지를 설계값, 측정값, 계산값으로 적었고, 측정값은 $`X_{ij}`$ 하나이며 나머지는 모두 거기에서 나온다.

- $`K`$: wafer 장수. 이 자료에서는 261. 설계값.
- $`N`$: wafer 한 장에서 재는 site 개수. 이 자료에서는 13. 설계값.
- $`M`$: 전체 관측치 개수이며 $`M = K N`$ 이다. 계산값.
- $`X_{ij}`$: $`i`$ 번째 wafer 의 $`j`$ 번째 site 에서 읽은 값. 측정값.
- $`\bar{X}_i`$: $`i`$ 번째 wafer 의 평균. 계산값.
- $`\bar{X}`$: 전체 $`M`$ 개의 총평균. 계산값.
- $`s_i`$: $`i`$ 번째 wafer 한 장의 site 값 $`N`$ 개로 계산한 표본표준편차. 계산값.
- $`\sigma_{\mu_n}`$: 처음 $`n`$ 장의 wafer 평균을 표본표준편차로 잰 값. 자료에서 관측한 산포이며 계산값.
- $`\hat{\sigma}_{\mu_K}`$: 같은 산포를 분산성분에서 계산한 값. 식 (12) 부터 식 (15) 까지의 좌변이며 계산값.
- $`s_i^2`$: $`i`$ 번째 wafer 안 site 값의 표본분산. within-wafer 성분. 계산값.
- $`S_{\mathrm{total}}^2`$: 전체 $`M`$ 개의 표본분산. 계산값.

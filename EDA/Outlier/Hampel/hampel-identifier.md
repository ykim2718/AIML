...
$$\mathrm{MAD} = \operatorname{median}_i \left| x_i - \tilde{x} \right|$$
...
| Scale | sd = 0.163469 | MAD / 0.674490 = 0.010823 |
| Upper boundary at 3.5 | 0.635053 | 0.061080 |

The scales differ by a factor of 15.1, and the whole of that gap is the single observation
0.6532 entering the standard deviation. The robust scale describes the other 14 observations;
the classical scale describes the outlier.
...
|---|---|---|---|
| Modified z | 58.2094 | 1.5800 | 54.7094 |
| Classical z | 3.6110 | 0.3029 | 0.1110 |

Both rules flag 0.6532 at a threshold of 3.5, so on this sample they agree. They do not agree on
how strongly.

The classical score reaches 3.6110 against a ceiling of 3.6148 for n = 15, which is 99.9% of the
...
same kind of data could not have flagged anything at all. The modified score reaches 58.2094 and
clears the threshold by 54.7094.

![Fig 1](hampel-identifier_fig/hampel_sample.png)

**Fig 1. The sample under both rules and the sensitivity of each to the threshold**

Panel (a) draws the two boundaries against the data on a log axis. The robust boundary sits at
0.0611, above every retained observation and well below 0.6532. The classical boundary sits at
0.6351, which is above 14 of the 15 observations because the outlier pushed it there; the flagged
point clears it by a margin too small to see at this scale.

Panel (b) scores every observation under both rules. The ceiling and the threshold are drawn as
two nearly coincident lines, which is the visual form of the previous paragraph: for this sample
size the classical rule has almost no room between the threshold it must clear and the largest
value it can produce.

Panel (c) sweeps the threshold. The modified score reports exactly one outlier for every
threshold from 1.58 to 58.21, a band spanning a factor of 37 that the conventional 3.5 sits well
inside. The classical score reports one only up to 3.61 and none above it, so a threshold of 3.5
sits 0.11 from flipping the answer.

### C.3. Breakdown in Motion
...
![Fig 2](hampel-identifier_fig/hampel_breakdown.png)

**Fig 2. What each rule reports as one observation is pushed outward**

Panel (a) is the breakdown point of section 2.2 made visible. The modified score grows without limit,
reaching 1845.8 when the contaminant is 20. The classical score rises at first, then flattens
...
The crossing near the left of panel (a) is worth noting. The classical score passes 3.5 when the
contaminant reaches 0.1385 and the modified score passes it at 0.0645, so the robust rule reacts
...
threshold of 3.5 on the classical score can never be reached. The sample of this appendix sits at

...
- **Assumption.** 정규성, 그리고 찾고 있는 것이 평균과 표준편차를 움직이지 못할 만큼 깨끗한 표본.
- **Setting.** 절대값 3 초과. 정규 표본에서 우연히 이를 넘는 것은 약 0.27% 이다.
- **Breaks when.** 오염이 있을 때. Outlier 가 자기가 견주어지는 척도를 부풀려 스스로를 가린다. 표본이 작을 때도 무너진다. 점수에는 $(n-1)/\sqrt{n}$ 의 상한이 있어 3 의 규칙은 관측 11 개 아래에서 발동하지 않는다 ([Appendix D](#appendix-d-estimator-constants-and-bounds)).
- **Met at.** 정규에 가깝다고 이미 알려진 단일 parameter 로, 큰 오류를 걸러 낸 데이터.
...
- **Assumption.** 분포의 형태는 가정하지 않되 대칭은 가정한다. Fence 가 양쪽에 같은 배수로 놓이기 때문이다.
- **Setting.** 안쪽 fence 는 1.5 IQR, 바깥쪽 fence 는 3 IQR ([Appendix B](#appendix-b-tukeys-rule)).
- **Breaks when.** 표본에 왜도가 있을 때. 긴 꼬리는 분포의 성질인데 규칙은 그것을 outlier 의 연속으로 읽는다 ([Appendix B](#appendix-b-tukeys-rule)).
- **Met at.** 단일 parameter 를 처음 들여다볼 때. Box plot 의 수염이 이미 이 규칙이다.
...
- $x_1, \ldots, x_n$ — 표본이고 $x_i$ 는 그 $i$ 번째 관측으로, 꼭지 4.1 과 같다.
...
- $M_i$ — 관측 $i$ 의 modified z-score 로, 꼭지 4.1 의 $z_i$ 와 같은 척도에서 읽는다.

- **Assumption.** 데이터에 대해서는 아무것도 가정하지 않는다. 정규성은 점수를 z-score 척도에 올리는 상수로만 들어온다 ([Appendix D](#appendix-d-estimator-constants-and-bounds)).
- **Setting.** Modified z-score 의 절대값 3.5 초과로, [Iglewicz and Hoaglin (1993)](#ref-14) 이 권한 값이다.
- **Breaks when.** 표본의 절반이 넘게 한 값을 취할 때. MAD 가 0 이 되어 점수가 정의되지 않는다.
- **Met at.** 자기 lot 안에서 판정하는 단일 parameter. 찾는 outlier 가 표본 안에 들어 있는 경우이다.
...
- **Assumption.** 표본에서 오염되지 않은 부분이 근사적으로 정규이고, outlier 개수의 상한 $r$ 을 데이터를 읽기 전에 정한다.
- **Setting.** $r$, 탐색 전체에 대한 유의수준, 그리고 [Rosner (1983)](#ref-13) 의 임계값 $\lambda_i$ ([Appendix D](#appendix-d-estimator-constants-and-bounds)).
- **Breaks when.** $r$ 을 실제 개수보다 작게 잡거나, 마지막으로 통과한 단계가 아니라 첫 실패에서 읽을 때. 그러면 masking 이 살아남는다.
- **Met at.** 표준에 견주어 근거를 대야 하는 판정. [ISO 16269-4](#ref-15) 의 다수 outlier 방법이다.
...
- $d^2(x)$ — 제곱 거리로, 변수가 하나일 때 꼭지 4.1 의 $z_i^2$ 로 줄어든다.

- **Assumption.** 다변량 정규성, 그리고 outlier 가 움직이지 않은 중심과 공분산.
- **Setting.** 변수마다 자유도 하나인 chi-square cut-off ([Appendix D](#appendix-d-estimator-constants-and-bounds)).
- **Breaks when.** $\mu$ 와 $\Sigma$ 가 오염된 데이터에서 나올 때. Outlier 가 뭉쳐 있으면 그것을 감추는 방향으로 $\Sigma$ 를 부풀린다. Robust 한 추정값이 이를 고친다 ([Appendix D](#appendix-d-estimator-constants-and-bounds)).
- **Met at.** 서로 상관된 몇 개의 parameter 를 함께 읽을 때. 한 공정 단계의 요약 parameter 같은 것이다.
...
- **Assumption.** Outlier 가 적고 성긴 영역에 놓여 있다는 것. 분포는 가정하지 않는다.
- **Setting.** Tree 의 개수와 부분표본 크기. 출력은 오류율이 아니라 순위이다.
- **Breaks when.** 이탈이 변수들의 조합에만 있을 때, 또는 contamination rate 을 짐작으로 정할 때.
- **Met at.** 행이 많고 변수가 넓은 표에서 첫 순위를 싸게 얻고자 할 때.
...
- **Assumption.** 밀도가 뜻을 가진다는 것. 곧 원래 좌표의 거리가 뜻을 가진다는 것이다.
- **Setting.** 이웃 크기 $k$. Factor 가 1 에 가까우면 평범하고, 1 보다 크게 높으면 성긴 자리이다.
- **Breaks when.** 변수가 많아 거리가 더는 가르지 못할 때, 또는 $k$ 가 cluster 경계를 넘어갈 때.
- **Met at.** 밀도가 다른 group 이 섞인 데이터. 여러 tool 이나 chamber 를 한 표에 모은 경우이다.
...
- **Assumption.** Outlier 는 적어도 한 변수의 꼬리 사건이라는 것.
- **Setting.** 없다. 여기서 정할 것이 없는 유일한 방법이다.
- **Breaks when.** 이탈이 변수들의 조합에만 있을 때. 꼭지 4.5 가 다루는 경우이다.
- **Met at.** 조율할 label 이 없는 넓은 표에서, flag 가 어느 변수 때문인지까지 대야 할 때.
...
시각 결함 검사는 하나의 처방으로 모였다. Pretrained network 를 image 위로 돌리고, 결함 없는 예시의 patch feature 를 보관하고, 새 patch 를 그 기억까지의 거리로 채점한다. 보고된 검출률과 지연은 [Appendix E](#appendix-e-benchmarks) 에 있다.

- **Assumption.** 결함 없는 예시가 정상의 모습을 덮고 있고, 제품이 되풀이된다는 것.
- **Setting.** Pretrained backbone, patch 크기, 기억의 크기, 그리고 거리 cut.
- **Breaks when.** 조명과 결함의 변이가 기억이 담은 것을 넘어설 때. 보고된 국소화 성능이 가파르게 떨어진다 ([Appendix E](#appendix-e-benchmarks)).
- **Met at.** 되풀이 생산되는 제품의 inline image 검사. Line 이 요구하는 지연 안에서 돈다.
...
- **SVM** — Support vector machine 으로, kernel 이 정한 기하 구조에서 얻을 수 있는 가장 넓은 여백으로 class 를 가르는 분류기. 꼭지 5.2 의 one-class 변형에는 두 번째 class 가 없어, 가진 하나를 감싼다.
...
꼭지 4.2 는 이 규칙을 한 줄로 진술한다. 이 appendix 는 1.5 라는 배수가 어디에서 왔는지, 그것이 z-score 에 견주어 무엇을 치르는지, 그리고 이 규칙이 어디에서 통하지 않게 되는지를 적는다.

### B.1. Inner and Outer Fences

[Tukey (1977)](#ref-1) 은 fence 를 한 쌍이 아니라 두 쌍 그렸다. 안쪽 쌍이 꼭지 4.2 의 규칙이고, 바깥쪽 쌍은 interquartile range 의 1.5 배가 아니라 3 배에 놓인다.
...
- $Q_1$, $Q_3$, $\mathrm{IQR}$ — 꼭지 4.2 와 같다.
...
1.5 라는 배수는 유도된 것이 아니라 편의로 고른 것이다. 꼭지 4.2 는 이 규칙을 3 의 z-score 와 비슷하다고 부르지만 둘은 같지 않다.
...
Fab 은 표준이 이름을 붙였고, 감사자가 확인할 수 있고, 기술자가 그에 따라 움직일 수 있는 방법을 돌린다. 그 가운데 둘은 꼭지 4 부터 6 까지에서 이미 다룬 구성이다.

### C.1. Part Average Testing

Part average testing 은 측정값이 모두 specification limit 을 통과하더라도 자기 lot 에서 비정상인 부품을 걸러 낸다. AEC-Q001 이 자동차 부품에 대해 꼭지 4.3 의 설계 위에 이를 정의한다. Robust mean 은 median 이고 robust sigma 는 interquartile range 를 1.35 로 나눈 것이며, 부품은 아래 구간 안에 들 때 남는다.
...
그 제수는 꼭지 4.2 의 $1.349 \sigma$ 를 반올림한 것이며, $\Phi^{-1}(0.75)$ 가 MAD 에 하는 일을 사분위수의 퍼짐에 한다. 표준은 MAD 대신 사분위수를, 3.5 대신 6 을 고르지만 구성은 같다. Robust 한 중심, 정규 단위의 robust 한 척도, 그 척도의 배수이다.
...
꼭지 4.1 부터 4.5 까지의 설정값은 본문이 유도 없이 적은 상수와 한계에 기대고 있다. 이 appendix 는 그것들이 어디에서 오는지를 적는다.

### D.1. The Ceiling on a Z-Score

Z-score 의 두 추정값은 모두 검정 대상 표본에서 나오므로, outlier 는 자기가 견주어지는 척도를 부풀린다. 같은 자기참조가 점수에 상한을 씌운다. 크기 $n$ 의 표본에서 어떤 z-score 의 절대값도 $(n-1)/\sqrt{n}$ 을 넘지 못하며, [Shiffler (1988)](#ref-12) 의 한계이다. 그래서 3 의 규칙은 관측 11 개, 3.5 의 규칙은 15 개 아래에서 발동할 수 없다.

### D.2. The MAD Consistency Constant

Raw MAD 는 $s$ 를 추정하지 않는다. 정규 표본에서 $0.674490 \sigma$ 로 수렴하여 퍼짐을 3 분의 1 가량 낮추어 말하며, $0.674490$ 은 표준정규분포의 3 사분위수이다. 이 값으로 나누는 것은 1.482602 를 곱하는 것과 같고, modified z-score 를 z-score 를 읽는 척도 위에 올린다. 그 단계가 없으면 어떤 문턱값도 두 규칙 사이를 오가지 못한다. 이 상수는 가정이 아니라 calibration 이다. 바꾸면 모든 점수가 같은 배율로 다시 재어질 뿐 순서는 그대로이다.

### D.3. Critical Values of the Generalized ESD

각 $R_i$ 는 그 단계에 맞추어 유도되어 [Rosner (1983)](#ref-13) 이 표로 만든 임계값 $\lambda_i$ 와 견준다. Outlier 의 개수는 $R_i \gt \lambda_i$ 가 되는 가장 큰 $i$ 이지 첫 번째가 아니다. 그렇게 읽는 것이 masking 을 이긴다. 한 단계가 실패해도, masking 을 일으킨 관측이 이미 제거된 뒤의 나중 단계는 성공할 수 있다.

### D.4. The Chi-Square Cut-Off and a Robust Covariance

다변량 정규성 아래에서 중심과 공분산이 추정값이 아니라 알려진 값이면, Mahalanobis 제곱 거리는 변수마다 자유도 하나인 chi-square 분포를 따르고 cut-off 는 거기에서 나온다. 오염된 표본에서 그 둘을 추정하면 이 읽기가 깨진다. Outlier 가 뭉쳐 있으면 그것들을 감추는 바로 그 방향으로 $\Sigma$ 를 부풀리기 때문이다. [Rousseeuw and Van Driessen (1999)](#ref-16) 의 minimum covariance determinant 가 robust 한 중심과 공분산을 주어 이를 되돌린다.

## Appendix E. Benchmarks

발표된 비교가 한 방법에 기대할 수 있는 몫을 정한다. 아래 수치는 그것을 낸 benchmark 에 속한다.

...
__author__ = 'yRocket'
__version__ = "0.13.2.2026.9.22"  # Semantic Versioning: Major.Minor.Patch.Date(YYYY.M.D)
...
def build_appendix_d_model() -> tuple:
    ...
    a = rng.normal(10.0, 0.7, n)                # knob
    b = rng.normal(5.8, 0.35, n)                # knob
    c = rng.normal(3.0, 0.3, n)                 # measured context
    d = 0.7 * c + rng.normal(0.0, 0.12, n)      # D follows C
    e = rng.normal(2.0, 0.2, n)
    t = 0.9 * a + 1.4 * b + 0.6 * c - 0.5 * e + 0.02 * a * b + rng.normal(0.0, 0.85, n)
...
def fig_7(out_folder: pathlib.Path) -> pathlib.Path:
    ...
    starts = data.iloc[(data['P'] - APPENDIX_D_TARGET).abs().nsmallest(APPENDIX_D_STARTS).index]
    constraints = [{'type': 'ineq', 'fun': lambda v: t2_limit - t2(assemble(v))},
                   {'type': 'ineq', 'fun': lambda v: spe_limit - spe(assemble(v))}]
    results = [minimize(objective, x0=row[APPENDIX_D_FREE].to_numpy(dtype=float), method='COBYLA',
                        constraints=constraints, options={'maxiter': 3000})
               for _, row in starts.iterrows()]
    best = min(results, key=lambda r: r.fun)
    x_starts = starts[APPENDIX_D_FREE].to_numpy(dtype=float)
    x_solved = best.x
    ...
    ax.scatter(t_column, p_column, s=18, color=COLORS[0], label='100 samples')
    ...
    ax.set_ylabel('P left behind by the hidden model', fontsize=font_size())
    ax.tick_params(labelsize=font_size(0.9))
    ...
    ax.scatter(x_starts[:, 0], x_starts[:, 1], s=60, marker='X', color=COLORS[1],
               label=f'{APPENDIX_D_STARTS} nearest sample starts')
    ax.scatter([x_solved[0]], [x_solved[1]], s=110, marker='*', color=COLORS[2],
               label=f'best solution, P = {p_solved:.2f}')
    ...
    near_pred = surrogate.predict(near[APPENDIX_D_FEATURES].to_numpy())

    ax = axes[3]
    span = [min(near_t.min(), near_pred.min()) - 0.6, max(near_t.max(), near_pred.max()) + 0.6]
    ax.plot(span, span, color='0.5', linestyle='--', linewidth=1.0, label='1:1 line')
    ax.scatter(near_t, near_pred, s=60, color=COLORS[2],
               label=f'{len(near)} rows within {APPENDIX_D_TOLERANCE:.0%} of the solution')
    ax.axhline(APPENDIX_D_TARGET, color=COLORS[3], linestyle=':', linewidth=1.2,
               label=f'inversion target {APPENDIX_D_TARGET:.1f}')
    ax.set_xlim(span)
    ax.set_ylim(span)
    ...
    panel_caption(fig, [axes[3]], f'(d) the rows run within {APPENDIX_D_TOLERANCE:.0%} of the solution')
    ...
    print(f'wrote {out_path}, hidden model R2 on T={r2_hidden:.4f}, '
          f'surrogate R2 on P={r2_surrogate:.4f}')
    print(f'  rows within {APPENDIX_D_TOLERANCE:.0%} = {len(near)}, '
          f'their true T = {np.round(np.sort(near_t), 2)}')

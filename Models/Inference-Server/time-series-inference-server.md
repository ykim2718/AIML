...
Rev. 9 | Created: 2026-08-28 | Updated: 2026-08-28 11:30 CDT
...
| A model fitted on demand | No | Two lesser things are available in its place. One is the bounded per-call adaptation of section 5.1, which fits the model further for that call alone. The other, where the model is recursive, is the state update of section 3.2, which advances one key's state and no version at all. | Nothing supplies a new fit inside a request, because a new fit changes what every other caller receives, so it belongs to a governed job. |
...
|----------|-------------|
| Actuals | The caller submits the observed values for a cut-off that was already answered, which is the input to scoring, and the submission is keyed by series and timestamp so that it is idempotent and a repeat cannot be counted twice. |
...
- Zero-shot: answering for a series with a pre-trained model that was never fitted to that series.

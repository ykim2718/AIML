...
학습된 여러 모델 가운데 배포에 사용할 모델을 정하는 축이며, Fig 1의 두 가지를 차례로 설명한다. 구현 예시는 [Appendix F](#appendix-f-python-examples-model-selection) 에 있다.
...
주의 — data leakage: $M_t$ 의 평가에는 학습 시점 $t$ 이후에 도착한 실제값만 사용해야 한다. 학습에 쓴 구간을 평가에 다시 쓰거나 validation 구간의 실제값이 학습 데이터에 섞이면 성능이 과대평가된다.

### 3.2 Online ensemble

Best model 하나를 유지하는 대신, 여러 모델의 예측을 실시간 성능에 따라 가중 결합하는 방식이다. River의 `model_selection`·`ensemble` module이 selection과 ensemble을 모두 지원한다.
...
River의 ADWIN detector가 예측 오차 stream의 분포 변화를 감지하면, 그때만 최근 window로 모델을 재학습한다. 주기적 재학습과 달리 drift가 없으면 재학습 비용이 들지 않는다.
...
# concept drift: the target relation flips halfway through the stream
X = rng.random((200, 3))
y = np.where(np.arange(200) < 100, 2 * X[:, 0], -2 * X[:, 0]) + 0.1 * rng.normal(size=200)
...
for t in range(TRAIN, len(y)):
    error = abs(y[t] - model.predict(X[t:t + 1])[0])
    detector.update(error)
    if detector.drift_detected:
        # retrain on the recent window only when the alarm fires
        model = LinearRegression().fit(X[t - TRAIN:t + 1], y[t - TRAIN:t + 1])

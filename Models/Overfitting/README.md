...
Rev. 0 | Created: 2026-09-06 | Updated: 2026-09-06 19:07 CDT

Wide data 는 열의 수 $p$ 가 행의 수 $n$ 에 견주어 크거나 아예 더 큰 자료를 말한다. 반도체 계측 자료, 유전자 발현 자료, 장비 sensor 요약값이 모두 여기에 든다. 이런 자료에서 overfitting 은 실수로 생기는 사고가 아니라 자료의 모양이 부르는 기본 상태이므로, 막을 장치를 미리 설계에 넣어야 한다. 이 문서는 그 장치를 원인, 증거, 방법, 검증의 순서로 정리한다.

## 1. Scope

- 다루는 범위: $p$ 가 $n$ 에 견주어 큰 자료에서 overfitting 이 생기는 이유와 그것을 줄이는 방법, 그리고 그 효과를 정직하게 재는 절차.
- 다루지 않는 범위: deep learning 고유의 정칙화, 시계열 고유의 검증 설계, 결측과 이상값 처리.
- 전제: 지도 학습이며 응답 $y$ 가 있고, 행은 서로 교환 가능하다.

## 2. Why Wide Data Overfits
...
셋째 원인은 절차에서 온다. 전체 자료를 보고 열을 고른 뒤 그 열로 cross-validation 을 돌리면, 고르는 단계에서 이미 검증 자료를 쓴 것이므로 오차가 낙관적으로 나온다. 이 편향은 열이 많을수록 커지며, 3 장의 그림이 보이는 규모의 우연 상관이 그 편향의 재료가 된다.

## 3. Evidence From A Simulation

앞 장의 세 원인 가운데 둘은 숫자로 바로 보인다. Fig 1 은 $n = 100$ 을 고정하고 열의 수만 늘리며 관찰한 것이다.

<img src="README_fig/wide-data-overfitting.png" width="1100" style="max-width: 100%;" alt="Fig 1">
...
두 panel 의 차이가 이 문서의 요점이다. **어떤 방어가 좋은지는 자료의 신호가 몇 개의 열에 모여 있는지에 달려 있으며, 방법 자체의 우열로 정해지지 않는다.**

## 4. Families Of Defenses
...
### 5.3 Choosing The Subspace Size
...
### 5.5 Relation To Bagging And Random Forest
...
## 7. Projection Before Regression
...
둘의 차이는 응답을 보는지에 있다. PCR 의 방향은 $y$ 를 보지 않고 정해지므로 분산은 크지만 예측에 쓸모없는 방향이 앞자리를 차지할 수 있고, 그러면 뒤로 밀린 성분 (component) 을 잘라 낼 때 신호까지 잘려 나간다. PLS 는 그 위험이 없는 대신 응답을 보았으므로, 성분 수를 정하는 일이 곧 model 복잡도를 정하는 일이 되어 8 장과 9 장의 절차가 더 중요해진다.

## 8. Feature Selection Without Selection Bias
...
## 10. Choosing Among Them

Table 4 는 자료의 모양에서 방법으로 가는 길이다.
...
순서는 늘 같다. 신호가 어디에 있다고 보는지를 먼저 정하고, 그에 맞는 방어를 고르고, 마지막으로 그 선택이 옳았는지를 정직한 절차로 확인한다. 세 단계 가운데 마지막을 건너뛰면 앞의 둘이 무엇을 했는지 알 수 없다.

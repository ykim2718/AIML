...
Rev. 0 | Created: 2026-08-27 | Updated: 2026-08-27 19:49 UTC
...
뒤의 명제는 맞다. 앞의 명제는 절반만 맞다. 학습에 쓴다는 것과 양산 중에 그 범위의 값이 잘 들어오지 않는다는 것은 맞지만, 두 가지가 어긋난다. 첫째로 wide DOE 로 학습한 model 자체는 추론에 그대로 쓰인다. 둘째로 wide 범위의 값이 들어오는 일이 아예 없지는 않으며, 들어오는 그때가 model 이 가장 필요한 순간이다.
...
| Inference | 드물게 일어난다 | 추론의 대부분을 차지한다 |
...
추론은 대부분 narrow 영역에서 일어난다. 양산 중에 장비를 극단 조건으로 돌리지 않으므로 들어오는 값이 POR 근처에 머물기 때문이다. Virtual metrology 로 계측값을 대신 내거나 FDC 로 이상을 가리는 일이 모두 이 구간에서 이루어진다.

그러나 wide 영역의 추론이 없는 것은 아니다. 장비가 흐르거나 이상이 나면 들어오는 값이 narrow 구간을 벗어난다. 그리고 그때가 model 이 옳은 답을 내야 하는 순간이다. Narrow 범위만 학습한 model 은 바로 그 순간에 extrapolation 을 하게 되어, 가장 필요한 때에 가장 못 미덥다. Wide DOE 가 값을 하는 자리가 여기다.

그러므로 앞의 명제는 이렇게 고쳐 읽어야 한다. Wide DOE 는 model 의 뼈대를 세우는 데 쓰이고 그 범위의 추론은 드물게 일어나며, 드물게 일어나는 그 추론이 가장 값진 추론이다.
...
Table 2. DOE in the machine learning workflow

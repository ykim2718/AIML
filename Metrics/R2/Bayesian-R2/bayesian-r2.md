...
Rev. 5 | Created: 2026-05-31 | Updated: 2026-09-04 14:57 CDT
...
The remedy proposed by Gelman et al. [[1](#ref-1)] is to replace the denominator with a sum
...
$$\mathrm{Var}(z) = \frac{1}{n-1}\sum_{i=1}^{n}\left(z_i - \bar{z}\right)^2 \hspace{19em} (1)$$
...
<a id="ref-1"></a>[1] Gelman, A., Goodrich, B., Gabry, J., & Vehtari, A. (2019). [R-squared for Bayesian Regression Models](https://doi.org/10.1080/00031305.2018.1549100). *The American Statistician*, 73(3), 307-309. Proposes the form section 3.2 uses, in which the denominator is the sum of the explained and the residual variance, so that the value stays inside [0, 1] for every posterior draw.
...
- **GBM**: Gradient Boosting Machine. A point-estimate model built as a sum of trees fitted in sequence.
...
- **Pearson R²**: The square of the Pearson correlation coefficient between the observations and the predictions. Both terms of that correlation are centered on their own mean and divided by their own standard deviation, so the value is invariant under any affine transform of the predictions with a non-zero slope. It therefore reports only whether the predictions move in step with the observations, never how close to them they are, and it is blind to bias and to scale. It coincides with standard R² for an OLS fit with an intercept, which is why the two are often taken to be one quantity, and it separates from standard R² for any prediction that is not the least-squares solution of that data.

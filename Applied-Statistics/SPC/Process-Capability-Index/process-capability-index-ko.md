...
$k$ 열은 이 규격을 한 가지가 아니라 두 가지에 거는 것으로 만들며, 두 번째가 무엇을 사는지는 식 (4)
가 말한다. 공정은 요구된 폭에 딱 맞춘 채 목표값 위에 정확히 앉아 있기만 해도 $C_{pk}$ 요구치를
만족하고, 그러면 평균이 움직이는 순간 미달이 된다. $k$ 에 상한을 두면 요구되는 $C_p$ 가
$C_{pk}/(1 - k)$ 로 올라가며, 그것이 Table 3 의 함의된 $C_p$ 열이고, 공정이 겪을 흐름을 지나면서도
$C_{pk}$ 를 지키는 데 필요한 여유이다.

함의된 폭은 등급 사이에서 별로 달라지지 않아 Priority 0 에서 1.86, Priority 2 에서 1.66 인 반면,
치우침의 허용치는 절반으로 줄어든다. 따라서 높은 등급이 요구하는 것의 대부분은 폭이 아니라
치우침이며, 이 선택은 의도된 것이다. 치우침은 setpoint 이고 폭은 hardware 이기 때문이다.

등급은 그 수가 모자랄 때 무엇을 하는지도 정한다. Priority 0 항목이 요구치에 못 미치면 lot 을 잡고
챔버를 생산에서 뺀다. Priority 1 항목은 lot 이 다음 공정으로 가기 전에 engineering review 를 거친다.
Priority 2 항목은 기록해 두었다가 다음 정기 정비에서 조치한다. 이 대응이 없으면 등급은 이름표에
그친다. 지수를 계산하는 방법은 세 행에서 모두 같기 때문이다.

이름과 정확한 수는 공표된 표준이 아니라 각 fab 안의 관례이며, 같은 fab 에서도 technology node 마다
다르다. 옮겨 가는 것은 그 꼴이다. 등급을 몇 개 두고, 각 등급에 $C_{pk}$ 의 최소값과 $k$ 의 최대값을
걸고, 각 등급에 조치 규칙을 붙인다.
...
- **Excursion**: 항목이 자리 잡은 거동에서 벗어난 것이며, 조치를 요구할 만큼 큰 것.

...
> 커진 경우를 excursion 으로 볼 것인지를 다룬다.
...
부분군이 커질수록 나머지 관측값을 버리는 대가가 커지고, 통상 $n \gt 10$ 이면 s 로 옮긴다
[[2](#ref-2)]. 부분군 크기가 매번 다를 때도 s 를 쓴다. 범위는 크기가 다르면 서로 비교할 수 없지만
$s$ 는 이미 $n$ 으로 나누어져 있기 때문이다.
...
두 관리도의 한계는 각 통계량의 중심에서 3 표준편차 떨어진 자리이며 [[3](#ref-3)], 식 (2) 를 넣으면
둘 다 $\bar{s}$ 의 배수가 된다. 그 배수가 관리도표의 상수이다.
...
읽으면 없는 원인을 쫓게 된다. 같은 부분군에서 s 관리도가 먼저 신호를 내고 있는지 확인하면 이 오독을
막을 수 있다.
...
두 경로 어느 쪽이든 산포가 흔드는 것은 평균값이 아니라 평균의 판정 능력이다.
...
| Method | What it reads | Conclusion |
|---|---|---|
...
넷째, 재현성이 깨져, 공정 조건을 안정시켜야 하기 때문이다. 산포가 커졌다는 것은 같은 조건에서 같은
결과가 다시 나오지 않는다는 뜻이고, 그런 조건 위에서는 관리한계도 $C_{pk}$ 도 다음 기간을 예고하지
못한다. Excursion
...
1️⃣첫째, $s$ 한 점의 변동이다. $s$ 자체의 상대 변동은 $c_4$ 하나로 적힌다.
...
2️⃣둘째, 부분군 구성의 변화이다. 측정점 수가 바뀌었거나 chamber 가 추가되었으면 부분군이 담는 변동원 자체가
달라진 것이므로, Table 4 의 마지막 행으로 먼저 확인한다.

3️⃣셋째, 측정계의 변화이다. 관측된 산포는 공정 산포와 측정 산포의 합이므로, gauge R&R 이 나빠졌으면 공정은
그대로여도 $s$ 가 커진다.

4️⃣넷째, baseline 추정의 불확실성이다. 관리한계가 짧은 기간에서 추정되었으면 $\bar{s}$ 자체가 낮게 잡혔을

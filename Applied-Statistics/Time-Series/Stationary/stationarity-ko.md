...
관측하는 동안 같은 계였다는 것이다. stationarity 는 그 전제에 붙인 이름이고, 현장의 말로
...
이 문서는 그 전제를 정리한다. 통계적 정의와 그 물리적 대응, 실무에서 만나는 stationarity 의 형태, stationarity 가
...
변하는 신호를 stationary 로 읽게 된다. 그런 신호에 스펙트럼 해석을 걸면 결과는 나오지만 그 결과가 어느
시점의 것인지 말할 수 없다.
...
다른 점은 그 열을 어떻게 다루었는가뿐이며, (a) 만 stationary 이고 나머지 셋은 각각 평균과 산포와 누적 구조에서
stationarity 를 잃는다. 굵은 선은 60 sample 창의 이동평균이고 띠는 같은 창의 이동표준편차이다.
...
하나를 stationary 로 읽게 된다. 식 (4) 가 어느 한 시차가 아니라 모든 시차의 관계를 조건으로 삼은 이유가
그림에 그대로 나와 있다.

### 2.3 Distinctions

stationarity 와 붙어 다니지만 같지 않은 것들이 있다. 이 셋을 구별해야 뒤의 논의가 어긋나지 않는다.

- 무상관과의 구별: stationary 과정은 자기상관을 가질 수 있다. 계수의 절댓값이 1 보다 작은 AR(1) 은 이웃한
  값끼리 강하게 묶여 있으면서도 stationary 이며, 값이 서로 무상관인 white noise 는 stationary 과정의 한 특수한
  경우일 뿐이다.
- Ergodicity 와의 구별: 시간 평균이 집단 평균과 같아지는 것은 stationarity 와 별개의 추가 가정이다. 실현마다
  다른 상수 offset 을 갖는 과정은 stationary 이면서 ergodic 이 아니다. 한 기록을 아무리 길게 재도 그 offset 은
  평균되어 사라지지 않는다.
- 규격과의 구별: stationarity 는 계가 어제와 같은가에 대한 것이고, 규격은 값이 허용 범위 안에 있는가에 대한
  것이다. 규격 안에 있으면서 non-stationary 인 공정이 있고, 규격을 벗어나 있으면서 stationary 인 공정도 있다.

## 3. Forms of Stationarity

실무에서 "stationary 이다" 라는 말은 서로 다른 여러 가지를 가리킨다. 어느 형태를 뜻하는지 정해 두어야
그다음에 고를 도구가 정해진다.
...
| 4 | Quasi-stationary | 짧은 창 안에서만 근사적으로 stationary | 운전 조건이 천천히 변하는 설비 |
| 5 | Difference-stationary | 차분하면 stationary. Unit root 보유 | 누적되는 drift. 계측기 offset 의 표류 |
| 6 | Trend-stationary | 결정론적 추세를 빼면 stationary | 선형으로 오르는 열 drift |
...
그 주기를 알면 위상별로 묶어 stationary 과정처럼 다룰 수 있다 [[2](#ref-2)]. Quasi-stationary 과정은 창을
짧게 잡아 그 안에서만 stationary 로 보는 취급이고, 시간-주파수 해석의 근거가 여기에 있다 [[3](#ref-3)].

## 4. Consequences for Analysis

### 4.1 Ergodicity

stationarity 위에 ergodicity 를 더하면 시간 평균이 집단 평균과 같아진다.
...
이것은 stationarity 에서 따라 나오는 것이 아니라 별도의 가정이다.

### 4.2 Spectral Analysis

신호를 FFT 로 주파수 영역에서 읽을 수 있게 해 주는 전제 조건이 stationarity 이다. stationary 과정에서
...
좋아진다는 보장도 사라진다. 그래서 non-stationary 신호에는 창을 짧게 끊어 각 창을 stationary 로 보는 STFT 나
wavelet 같은 시간-주파수 기법을 쓴다.

### 4.3 Transfer of a Model Across Time

과거 자료로 정한 것이 미래에도 유효하다는 보장 역시 stationarity 에서 나온다. 필터 계수, 관리한계, 회귀
모델의 계수는 모두 추정할 당시의 통계량을 담고 있으므로, 그 통계량이 변하면 값 자체는 그대로인 채
근거만 없어진다. 재현성과 예측 가능성을 stationarity 의 결과로 묶어 두는 이유가 이것이다.

## 5. Assessment on a Finite Record

### 5.1 The Observation Window

판정에 앞서 창의 길이를 정해야 한다. stationarity 는 자료가 절대적으로 갖는 성질이 아니라 관측 segment 에
상대적인 성질이기 때문이다. 1 초 창에서 stationary 인 진동이 8 시간 창에서는 온도 drift 때문에 non-stationary 가 되고,
한 lot 안에서 stationary 인 계측값이 분기 단위로 보면 추세를 갖는다. "이 신호는 stationary 인가" 는 답할 수 없는
물음이고, "이 신호는 이 창에서 stationary 인가" 가 답할 수 있는 물음이다.
...
| 5 | KPSS test | stationary | ADF 단독으로는 갈리지 않는 경우 |
...
$\gamma = 0$ 이면 식 (5) 의 누적 구조가 남아 있다는 뜻이므로, 귀무가설의 기각이 stationarity 쪽의 증거가
된다. KPSS test 는 귀무가설을 반대로 놓아 stationarity 를 귀무가설로 삼는다 [[6](#ref-6)].

귀무가설이 서로 반대이므로 둘을 함께 돌려 네 가지 조합으로 읽는다. ADF 를 기각하고 KPSS 를 기각하지
못하면 stationary 로 본다. 그 반대이면 unit root 가 있는 것으로 본다. 둘 다 기각하면 결정론적 추세와
...
소모품의 수명을 알려 준다. stationarity 를 확인하는 일의 목적은 자료를 stationary 로 만드는 데 있지 않고, 지금
...
- **KPSS test**: stationarity 를 귀무가설로 놓는 검정. ADF test 와 반대 방향에서 같은 물음을 본다.
...
- **quasi-stationarity**: 짧은 segment 안에서만 근사적으로 stationary 인 성질.
...
- **trend-stationary**: 결정론적 추세를 제거한 뒤에 stationary 가 되는 성질.
...
여기서 stationarity 는 설비가 정속 운전 (steady-state operation) 중이라는 뜻이다. 회전수와 부하가 일정한
동안 발생하는 진동과 음향은 통계량이 변하지 않고, 그래야 그 신호의 주파수 성분을 설비의 고유한
특성으로 읽을 수 있다. 베어링이나 기어에 결함이 생기면 이 스펙트럼 패턴이 달라지고, 상태 진단은
그 차이를 본다.

한 가지 단서를 붙여야 한다. 회전기계의 진동은 엄밀히는 stationary 과정이 아니라 회전 주기에 묶인
...
있어 수십 년 기록 전체를 하나의 stationary 과정으로 보기 어렵다. 실무는 그래서 자료를 sea state 처럼
조건이 고른 segment 로 나누어 각 segment 를 stationary 로 취급하고, segment 별 손상을 발생 빈도로 가중해 합친다.

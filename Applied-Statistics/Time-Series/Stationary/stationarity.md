...
Rev. 0 | Created: 2026-09-07 | Updated: 2026-09-07 10:25 CDT

> 계측 데이터의 정상성 (stationarity) 에 대한 기록. 통계적 정의와 물리적 읽기, 실무에서 만나는 여러
> 형태, 신호처리와 상태진단과 구조신뢰성에서 그것이 무엇을 보장하는지, 그리고 유한한 기록 하나로
...
어디인지가 아니라 그 간격에만 의존한다는 것이다. 분산은 $\sigma^2 = R(0) \lt \infty$ 로 유한해야
한다.

여기서 흔한 오해 하나를 짚어 둔다. 정상성을 평균과 표준편차가 일정한 상태로만 기억하면 식 (3) 이
빠진다. 평균과 분산이 일정하면서도 상관 구조가 시간에 따라 변하는 신호는 얼마든지 있고, 그런 신호에
스펙트럼 해석을 걸면 결과는 나오지만 그 결과가 어느 시각의 것인지 말할 수 없다. 정상성이 요구하는
것은 세 수치가 아니라 상관 구조 전체의 시간 불변성이다.
...
정상성과 붙어 다니지만 같지 않은 것들이 있다. 이 넷을 구별해야 뒤의 논의가 어긋나지 않는다.

- 무상관과의 구별: 정상 과정은 자기상관을 가질 수 있다. $|\phi| \lt 1$ 인 AR(1) 은 이웃한 값끼리 강하게
  묶여 있으면서도 정상이다.
- White noise 와의 구별: white noise 는 정상 과정의 한 특수한 경우일 뿐, 정상성의 정의가 아니다.
...
Cyclostationary 과정은 정상성이 깨진 것이 아니라 주기라는 구조를 하나 더 가진 것이며, 그 주기를
알면 위상별로 묶어 정상 과정처럼 다룰 수 있다 [[2](#ref-2)]. Quasi-stationary 과정은 창을 짧게
...
함수가 된다. 결함 성분이 평균 스펙트럼에 잘 나타나지 않으면서 포락선 스펙트럼에서 선명하게 보이는
이유가 여기에 있다 [[4](#ref-4)].

### 4.3 Structural and Reliability Engineering

여기서 정상성은 구조물에 가해지는 하중의 통계적 가혹도가 일정하다는 뜻이다. 교량이나 해상 구조물의
설계는 파랑과 바람을 확정된 시간 이력이 아니라 random vibration 으로 다루고, 하중의 power spectral
...
이것은 정상성에서 따라 나오는 것이 아니라 별도의 가정이며, 실현마다 고정된 개체 차이가 있는 자료에는
성립하지 않는다.
...
- **autocovariance**: 한 신호의 두 시점 값 사이의 공분산. 정상 과정에서는 두 시점의 간격만의 함수이다.
...
- **transient**: 정상 상태에 이르기 전이나 조건이 바뀌는 동안 나타나는 과도 구간의 신호.

...
이 절은 automatic rule loading에 필요한 설정을 다룬다. marketplace repository를 만들고(4.1), 각 project의 settings file에 등록한다(4.2). 적는 내용은 하나이며, 그 내용을 어느 settings file에 두는가에 따라 적용 범위가 달라진다. 기준이 되는 위치는 project settings이고, 범위별 적용 여부는 3.1절의 표에 정리되어 있다.

### 4.1 Git Repository

rule을 담는 marketplace repository는 일반 git repository이며, 최소 구성은 manifest file 두 개이다.

...
- **`/plugin marketplace add <owner>/<repo>`**: plugin marketplace를 등록한다.
- **`/plugin install <plugin-name>@<marketplace-name>`**: plugin을 설치한다.
- **`/plugin marketplace update <marketplace-name>`**: marketplace와 plugin을 수동으로 갱신한다.
...
`installed_plugins.json` — 설치된 plugin의 version, 설치 시각, 설치 당시의 commit을 기록한다:
...
```json
{
  ...
  "plugins": {
    "yrocket-rules@claude-configuration": [
      {
        "scope": "user",
        "installPath": "C:\\Users\\Asus\\.claude\\plugins\\cache\\claude-configuration\\yrocket-rules\\0.1.0",
        "version": "0.1.0",

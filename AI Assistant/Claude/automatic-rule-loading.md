...
1. plugin이 새 session 시작 시 최신 상태로 update 된다. Desktop interface에서는 [3.2](#32-session-start-hook-in-desktop-interface) 의 hook을 한 번 갖추어야 이 역할이 돌아간다.
...
💡 **`"autoUpdate": true` 는 marketplace를 GitHub 최신 기준으로 갱신하라는 지시이다.** 나머지 field는 무엇을 받을지 가리킬 뿐이고, 갱신 대상을 정하는 것은 이 값이다. Desktop interface에서는 `DISABLE_AUTOUPDATER=1` 때문에 이 지시가 실행되지 않지만, 같은 file을 Web에서도 쓰고 Desktop app이 이 제약을 거두면 그대로 동작하므로 적어 둔다. 실행되지 않는 동안의 갱신은 [3.2](#32-session-start-hook-in-desktop-interface) 의 hook이 맡는다.
...
`add` 와 `install` 두 명령을 terminal에서 실행하거나 prompt에 적어 Claude에게 시키면 각각 marketplace 등록과 plugin 설치가 이루어지며, 결과는 settings file이 아니라 [Appendix E](#appendix-e-plugin-state-files-desktop) 의 상태 file에 기록된다. CLI 설치는 [Appendix B](#appendix-b-claude-cli-desktop) 를 본다.
...
💡 SessionStart hook이 `update` 두 명령을 실행하면 사람이 개입하지 않아도 session을 열 때마다 갱신이 이루어진다. Desktop interface에서 autoUpdate를 대신하는 자리이다.
...
Show the active <SKILL_NAME> skill.
```
...
- 갱신은 지금 열려 있는 session이 아니라 다음 session부터 적용되므로, push와 적용 사이에 한 session의 지연이 남는다.
...
- **Plugin marketplace**: marketplace manifest와 plugin들을 함께 담은 git repository 전체이다. rule은 이 안에서 관리되며, 사람은 local repository에서 고쳐 push 하고 각 interface는 remote repository에서 내려받아 쓴다.
...
plugin 관련 명령들이며, terminal에서 실행하거나 prompt에 적어 Claude에게 시킨다.
...
`/plugin` 명령은 Desktop interface의 prompt에서 동작하지 않을 수 있다. 그때는 [Appendix B](#appendix-b-claude-cli-desktop) 의 CLI 명령을 prompt에 적어 Claude에게 시킨다. Web interface에는 `/plugin` 도 CLI도 없으므로 이 방법을 쓸 수 없고, 확인은 [4.3](#43-session-content-web) 을 본다.

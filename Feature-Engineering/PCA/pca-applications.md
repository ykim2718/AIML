# PCA Applications
Rev. 4 | Created: 2026-08-11 | Updated: 2026-08-11 14:46 CDT

> Knowing the lineage and deciding what to run are two different jobs.
> This document first groups the directions the extensions took, then says which branch actually gets used for each kind of semiconductor measurement data, and closes with a table that goes from a data condition to a method.
...
|---|---|---|
| Nonlinear and kernel | The structure does not separate in a linear subspace | Kernel PCA |
| Robustness and missing data | Outliers are mixed in and values are missing | Robust PCA, Probabilistic PCA |
| Complex data structure | Unfolding into a matrix destroys the structure | Sparse PCA, Tensor PCA, Functional PCA |
| High-dimensional and large data | Memory and computing time are the limit | Randomized PCA, Incremental PCA |
...
**Complex data structure.** Sparse PCA cuts down the variables a component uses so that the component can be read. Tensor PCA reduces a multi-way array without unfolding it. Functional PCA treats a continuous curve as a single unit, and it has been carried further into contrastive forms that look for curve variation peculiar to one group against a background.
...
Classical PCA assumes that the data is a plain matrix, that samples outnumber variables, that no outliers are present, and that the structure is a linear subspace. Each kind of measurement data violates a different one of those assumptions, and the violated one decides which branch applies.

Table 2. What each data type breaks

| Data type | Shape | Assumption it breaks |
|---|---|---|
| Wafer metrology | wafer × measured item | Samples are few and the items are strongly correlated |
| FDC trace | wafer × sensor × time | It is not a matrix, and `p` far exceeds `n` |
| Wafer map | wafer × die grid | It is a two-dimensional array whose neighbours carry meaning |
| DOE result | condition × response | Samples are placed by design, so variance is not evidence |
| Marathon test | time × sensor | The subspace itself moves as time passes |

Here `n` is the number of samples, that is the number of rows, and `p` is the number of variables, that is the number of columns. If one wafer is one row, `n` is the number of wafers; if one measured item is one column, `p` is the number of items. A dataset with `p` above `n` is called wide, and in that state there are more quantities to estimate than samples to estimate them from, so the sample covariance does not reach full rank.
...
The second is the sample count. Twenty-five wafers in a lot is not enough to estimate components stably. Eigenvalue shrinkage, or a conservative choice of how many components to keep, is needed, and the count should be confirmed by cross-validation rather than read off a scree plot alone.
...
When data from several tools is gathered into one table, the difference between tools becomes the largest variance and takes the first component. That component is not process variation; it is the tool identifier, and leaving it in pushes the later components down.
...
In long-run data the subspace itself moves slowly. Projecting onto fixed components leaves a residual that grows over time, and whether that growth is model decay or a process change is the question that splits the handling.
...
|---|---|---|---|
| The data is a curve and its smoothness is information | Functional PCA, or parameterize then PCA | Structure | §4 |
| The data has three or more axes | Tensor or Multilinear PCA | Structure | §4 |
| A spatial pattern on a grid is information | 2DPCA, or a treatment that states spatial correlation | Structure | §5 |
| Outliers are always present | Robust PCA or L1-PCA | Robustness | §1 |
| Many values are missing | Probabilistic PCA | Robustness | §1 |
| `p` far exceeds `n` | Eigenvalue shrinkage and a conservative component count | Scale | §3 |
| The data does not fit in memory | Randomized or Incremental PCA | Scale | §1 |
| The data arrives as a stream | An incremental or streaming branch | Scale | §7 |
...
| A tool effect takes the first component | Per-tool normalization or contrastive PCA | — | §6 |

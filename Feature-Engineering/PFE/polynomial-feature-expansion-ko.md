...
Rev. 38 | Created: 2026-09-07 | Updated: 2026-09-11 21:35 CDT
...
조건수를 올리는 것은 degree 와 열 사이의 collinearity 이고, 내리는 것은 centering 과 standardization 이다. Centering 은 열 사이의 겹침을 줄이고 standardization 은 열마다 다른 크기를 없애므로, 둘을 함께 하면 조건수가 가장 낮아진다. 4.2 절과 같은 표본에서 $d = 2$ 의 design matrix 조건수는 원 단위에서 $1.6 \times 10^5$, centering 과 standardization 뒤에는 2.8 이다. $d = 4$ 에서는 $3.4 \times 10^{10}$ 과 16 이고, $d = 8$ 에서는 $1.5 \times 10^{21}$ 과 $8.0 \times 10^{2}$ 이다 (Fig 1(b)). 64-bit 실수의 유효 자릿수가 약 16 자리이므로, 원 단위의 $d = 8$ 에서는 풀어 얻은 계수에 유효 숫자가 하나도 남지 않는다.

...
Rev. 43 | Created: 2026-09-07 | Updated: 2026-09-11 22:31 CDT
...
Centering 의 첫 번째 이유는 상관의 감소다. 물리 단위의 값은 대개 0 에서 멀리 떨어져 있고, 그런 $x$ 와 $x^2$ 는 거의 같은 방향을 가리킨다. 평균을 뺀 값을 $u = x - \bar{x}$ 로 두면 $x^2 = u^2 + 2\bar{x}u + \bar{x}^2$ 이므로, 상관의 분자인 공분산이 식 (6) 으로 갈라진다.

...
Rev. 40 | Created: 2026-09-09 | Updated: 2026-09-11 22:22 CDT
...
Values in physical units usually sit far from zero, and such an $x$ and $x^2$ point in nearly the same direction. Writing the deviation from the mean as $u = x - \bar{x}$ gives $x^2 = u^2 + 2\bar{x}u + \bar{x}^2$, so the covariance that forms the numerator of the correlation splits as equation (6).

$$\mathrm{cov}(x, x^2) = 2\bar{x}\,\mathrm{var}(u) + \overline{u^3} \hspace{19em} (6)$$

Over 60 samples on $[10, 11]$, $\bar{x} = 10.51$, $\mathrm{var}(u) = 0.0880$ and $\overline{u^3} = -0.0034$, so the covariance is $1.8494 - 0.0034 = 1.8459$; the standard deviations of $x$ and $x^2$ are $0.2967$ and $6.2224$, and the correlation is $1.8459 / (0.2967 \times 6.2224) = 0.9999$. After centering $\bar{x} = 0$ drops the first term and leaves $\overline{u^3}$ alone, so the correlation is $-0.0034 / (0.2967 \times 0.0782) = -0.15$. The first term being 540 times the second is what takes the correlation in physical units to 1, and after centering the correlation is proportional to the third central moment $\overline{u^3}$, so it is zero for a symmetric distribution and lands near zero in a sample.
...
Substituting the shift $x_1 = z_1 + a$, $x_2 = z_2 + b$ into a product-only model such as $y = \beta_{12} x_1 x_2$ gives equation (7).
...
Equation (4) defines the set of columns to be built without saying how large it is. That size is equation (8) and equation (9), derived below.

One monomial of degree exactly $k$ corresponds to one choice of non-negative integer exponents $(a_1, \dots, a_n)$ summing to $k$, so counting the monomials of that degree is counting those choices. That count is equation (10), whose left side carries a pair of bars $\lvert \cdot \rvert$ for the number of elements in the set they enclose.
...
Summing the degrees from 0 to $d$ gives equation (11). Writing it with one slack exponent $a_0 \ge 0$ such that $a_0 + \sum_i a_i = d$ collapses the sum into a single count, that of $d$ items falling into $n+1$ bins.

$$\sum_{k=0}^{d} \binom{k+n-1}{n-1} = \binom{n+d}{d} \hspace{19em} (11)$$

The set of equation (4) excludes the constant term at $k = 0$, so its size is $\binom{n+d}{d} - 1$, which is equation (8).

With `interaction_only` no variable is used twice, so a surviving term corresponds to one subset of the $n$ variables of size $j$, where $j$ runs from 1 to $\min(d, n)$. Adding those counts is equation (9). Once $d \ge n$ every subset is admitted and the sum closes as equation (12).

$$\sum_{j=1}^{n} \binom{n}{j} = 2^n - 1 \hspace{19em} (12)$$

## Appendix C. Ridge And Lasso On Expanded Columns

The penalty on the expanded columns is one of three. Written as an objective, ridge is equation (13) and lasso is equation (14) [[12](#ref-12)], where $\alpha$ sets how hard the penalty presses.
...
Expanded columns are far from orthogonal (section 4.2) and come in groups that resemble one another. Ridge spreads one coefficient across such a group; lasso keeps one member and zeroes the rest, and which member survives changes with the sample, so the list of terms lasso returns is itself unstable. Elastic net, equation (16) [[13](#ref-13)], mixes the two by $\rho$, which is lasso at 1 and ridge at 0. Its quadratic part keeps a group in or out together, so terms are still selected while the list moves less.
...
The cost of an expansion is linear in the column count, and that count grows by equation (8). At 100,000 rows, 100 variables and $d = 2$ the columns number 5,150, and holding them in a dense matrix, one that stores every value, takes 4.1 GB at 64 bits a value. Two routes keep the expanded columns out of memory.

The first is the kernel. The polynomial kernel of equation (17) computes the inner product of the expanded space without the expansion.

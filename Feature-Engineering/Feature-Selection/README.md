# Feature Selection
Rev. 9 | Created: 2026-09-12 | Updated: 2026-09-20 09:50 CDT
...
| [BIC/bic.md](BIC/bic.md) | It defines the Bayesian information criterion as the deviance of a fit plus a penalty of the log of the sample size per parameter, derives that penalty from the marginal likelihood, gives the linear regression form and the conditions that break it, fixes what a difference between two scores is worth, and closes on four appendices — one carrying that derivation down to the bounded terms it drops, one setting BIC against AIC as an identification criterion against a prediction criterion, one deriving the deviance of a Gaussian linear model from its likelihood, one linking a forward selection run kept in `src/` where BIC stops on the three generating features and AIC on five. |
| [BIC/bic-ko.md](BIC/bic-ko.md) | It is the Korean edition of the BIC document, carrying the same derivation, tables, worked example and references. |
...
4. If a selection path is already in hand and the size it should stop at is open, read the BIC document.

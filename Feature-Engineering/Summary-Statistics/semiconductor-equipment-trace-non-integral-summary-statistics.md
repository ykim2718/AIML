...
Fig 1. Sine 주파수와 잡음 수준에 따른 Roughness 축 세 특징. 열은 왼쪽부터 1, 2, 5, 12, 30 주기이고, 행은 위에서부터 잡음 표준편차 0, 0.05, 0.20, 0.60, 2.00이다. 각 panel에 그 파형에서 계산한 `slsr`, `zcr`, `cycle_count` 를 적었다.

`slsr` 에서 읽어야 할 것은 세 가지다.

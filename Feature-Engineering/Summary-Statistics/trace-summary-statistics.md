...
- $x(t)$, $t \in [0, T]$ 이고 sample 수는 $N \approx 300$ 이다.
...
|---|---|
| 시간축 $T$ | 요약통계와 적분 특징 |
| Sensor축 (200) | 상수·중복 channel pruning, taxonomy 그룹 집계 |
| 최종 특징축 $p$ | Sparse group lasso, PLS 등 지도학습적 축소 |

Sensor축으로는 concatenate하지 않는다. Sensor당 $d$ 차원 표현을 200개 이어붙이면 어떤 방법이든 wide로 붕괴한다. Sensor축은 반드시 집계 (pool) 하거나 sensor당 소수의 스칼라로 줄인다.
...
after channel pruning :  70 sensors x 4 features =   280  -> p/n = 0.93  (still too many)
taxonomy pooling      :                             ~100
...
0차만 쓰면 형상 정보가 전부 소실되고, 차수를 무한정 올리면 잡음만 증폭된다. $T \approx 300$, $n \approx 300$ 조건에서는 0–2차와 위상 하나의 조합이 정보 대비 분산의 절충점이다.
